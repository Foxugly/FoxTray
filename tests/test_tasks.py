...
from foxtray import process, tasks

# Use the real kill_tree so test processes are actually terminated. Fake
# kill_tree stubs would leak 2-10 second Python sleepers under watcher-mode
# pytest runs.
_REAL_KILL_TREE = process.ProcessManager().kill_tree
...
def test_run_second_time_same_key_raises(
    ...
    tm = tasks.TaskManager(kill_tree=_REAL_KILL_TREE, on_complete=cb)
    tm.run("task:A:Sleep", _python_sleep(2), tmp_path)
    with pytest.raises(tasks.TaskAlreadyRunning):
        tm.run("task:A:Sleep", _python_sleep(2), tmp_path)
    tm.kill_all()  # cleanup — real kill_tree terminates the sleeper
...
def test_kill_all_kills_tracked_popens_and_returns_count(
    ...
    # Record each PID the callback sees, while still delegating to the real
    # kill_tree so the sleeper processes are actually terminated.
    killed_pids: list[int] = []
    def _kill_and_record(pid: int) -> None:
        killed_pids.append(pid)
        _REAL_KILL_TREE(pid)
    cb, _done, _completions = _collect_completions(2)
    tm = tasks.TaskManager(kill_tree=_kill_and_record, on_complete=cb)
...
def test_on_complete_exception_does_not_crash_watcher(
    ...
    tm = tasks.TaskManager(kill_tree=_REAL_KILL_TREE, on_complete=_bad_cb)
    tm.run("task:A:Test", _python_exit(0), tmp_path)
